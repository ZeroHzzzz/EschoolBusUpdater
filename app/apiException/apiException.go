...
var (
	...
	AuthWrong                 = NewError(http.StatusInternalServerError, 200503, "身份验证失败")
	NoThatPasswordOrWrong     = NewError(http.StatusInternalServerError, 200504, "密码错误")
	HttpTimeout               = NewError(http.StatusInternalServerError, 200505, "请求超时，请稍后重试!")
	RequestError              = NewError(http.StatusInternalServerError, 200506, "请求异常，请稍后重试!")
	ResponseError             = NewError(http.StatusInternalServerError, 200506, "响应异常，请稍后重试!")
